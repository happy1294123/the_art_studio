...
export default function LinkGroup({ handleNavigate }: Props) {
  ...
  return (
    <div className="text-xl md:flex gap-5 text-fontColor">
      <div className="hover:bg-secondary md:hover:bg-secondary rounded-xl p-2 duration-300 cursor-pointer md:h-auto" onClick={() => handleNavigate('/')}>最新消息</div>
      <Collapsible className="hover:bg-secondary md:hover:bg-secondary rounded-xl duration-300 cursor-pointer">
      ...
      <div className="hover:bg-secondary md:hover:bg-secondary rounded-xl p-2 duration-300 cursor-pointer" onClick={() => handleNavigate('/')}>合作品牌</div>
    </div>
  )
}
